...
> Only PDF Editor that I need, and it's free!
...
<img src="screenshot/pdf_masaic.png" alt="Mosaic redaction" width="800">

Figure 1 shows mosaic mode. Drag on the page to mask text. The top area shows pixel blocks. Use the toolbar to select move, mosaic, blackout, or whiteout.

### 2. Blackout redaction

<img src="screenshot/pdf_censors.png" alt="Blackout list" width="800">

Figure 2 shows blackout mode. The black box hides text. The right panel lists all masks. It shows page, type, and size.

### 3. Page manager

<img src="screenshot/pdf_manager.png" alt="Page manager" width="800">

Figure 3 shows page manager. Drag cards to change order. Use icons to rotate, split, or delete. Press the blue button to save merged PDF.

### 4. Size optimizer

<img src="screenshot/pdf_compressed.png" alt="Size optimizer" width="800">

Figure 4 shows size optimizer. Enter target size in MB. Use presets for 75%, 50%, or 25%. Press save to write new file.
...
1. 태그 없는 일반 `push` / `pull_request` ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)):
   ...
   - 빌드 테스트 전용: 단위 테스트(Node.js & Rust)를 수행하고 크로스 플랫폼(Windows, macOS, Linux) 앱 빌드를 검증하여 아티팩트로 저장합니다 (릴리스는 작성되지 않음).

2. 버전 태그 지정 후 `push` 시 자동 릴리스 ([`.github/workflows/release.yml`](.github/workflows/release.yml)):
   - `v*` 형식의 Git Tag를 생성하고 push하면 GitHub Releases가 자동으로 작성 및 발행됩니다.
